...
preprocess_hgnc_database <- function(){
  ...
  hgnc_database.df <- read.csv(hgnc_database.path, header = TRUE, sep = "\t") %>% dplyr::tibble()
  # hgnc_database.df
  ...
  hgnc_database_all_symbols_in_one_colum.df <- hgnc_database.df %>% dplyr::mutate(
    ...
    dplyr::mutate(
      HGNC.SYMBOL.ALL=gsub(pattern = " ", replacement = "", x = HGNC.SYMBOL),
      HGNC.SYMBOL=gsub(pattern = ",,", replacement = ",", x = HGNC.SYMBOL.ALL),
      HGNC.SYMBOL=gsub(pattern = ",$", replacement = "", x = HGNC.SYMBOL.ALL)
    ) %>%
    dplyr::select(HGNC.SYMBOL, tidyr::everything())
  ...
  hgnc_database_long.df <- hgnc_database_all_symbols_in_one_colum.df %>%
    tidyr::separate_rows(HGNC.SYMBOL, sep = ",") %>%
    dplyr::distinct(HGNC.SYMBOL, .keep_all = TRUE) %>%
    dplyr::filter(nzchar(HGNC.SYMBOL))
